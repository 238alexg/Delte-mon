    ...
    public class ItemsUI : UIScreen
    {
        ...
        public void UseItem()
        {
            ...
            if (UIManager.Inst.inBattle)
            {
                ...
                if ((activeItem.itemT == itemType.Ball) || (activeDelt == BattleManager.Inst.State.PlayerState.DeltInBattle))
                {
                    ...
                    BattleManager.Inst.ChooseItem(activeItem);
                }
                ...
                else
                {
                    ...
                    UIManager.Inst.StartMessage(null, null, () => BattleManager.Inst.ChooseItem(activeItem));
                }
            }
            else
            {
        ...
        public IEnumerator healDeltemon()
        {
            ...
            while (healthBar.value < activeDelt.health)
            {
                ...
                // REFACTOR_TODO: Move this color reference to somewhere more general
                // Set colors for lower health
                if ((healthBar.value >= (activeDelt.GPA * 0.5f)) && (healthBarFill.color != BattleManager.Inst.BattleUI.fullHealth))
                {
                    healthBarFill.color = BattleManager.Inst.BattleUI.fullHealth;
                }
                else if ((healthBar.value >= (activeDelt.GPA * 0.25f)) && (healthBarFill.color != BattleManager.Inst.BattleUI.fullHealth))
                {
                    healthBarFill.color = BattleManager.Inst.BattleUI.halfHealth;
                }
